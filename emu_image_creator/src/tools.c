...
void gen_subq(uint8_t track_mode, uint8_t track_num, uint32_t abs_sect, uint32_t rel_sect, uint8_t subq[12]) {
    ...
    subq[0] = (track_mode == 0) ? 0x21 : 0x41;
    subq[1] = track_num;

    subq[2] = (rel_sect < 150) ? 0 : 1;

    if(rel_sect < 150) {
        lba_to_msf(150 - rel_sect, &min, &sec, &frame);
    } else {
        lba_to_msf(rel_sect - 150, &min, &sec, &frame);
    }

    subq[3] = bcd_to_hex(min);
    subq[4] = bcd_to_hex(sec);
    subq[5] = bcd_to_hex(frame);

    subq[6] = 0x00;

    lba_to_msf(abs_sect, &min, &sec, &frame);
    subq[7] = bcd_to_hex(min);
    subq[8] = bcd_to_hex(sec);
    subq[9] = bcd_to_hex(frame);
